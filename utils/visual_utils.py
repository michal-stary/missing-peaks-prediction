...
import numpy as np
from utils.helpers import get_top_k_ind
...
def plot_spectrum_sample_prediction(proc_spec, missing_mz, TP_mz, FP_mz, FN_mz, max_mz=1001, save_to_path=None):    
    ...
    sns.scatterplot(x=mz_FP, y=dots, ax=ax1, hue=np.tile(hue[hue=="FP"],5), palette=palette, legend=False, marker="x", s=10*200/(focus[1]-focus[0]))
    
    plt.sca(ax1)
    
    ax1.title.set_text(f"{proc_spec.metadata['compound_name']}")
    ax1.set_xlim(*focus)
    ax1.xaxis.set_tick_params(labelbottom=True)
    ax1.set(ylabel='intensity')
    ax1.set(xlabel='m/z')
