...
import numpy as np
from utils.helpers import get_top_k_ind, get_mz_vector, get_his_size
...
class TopKDS(Dataset):
    def __init__(self, ref_docs, vocab, k=5, onehot=True, add_intensity=True,\
                 mode="one", onehot_y=False):
...
class HuggDS(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        #y_ind = ordered_ind[1:]
        if self.add_intensity:
            position_ids = torch.tensor(np.digitize(self.ref_docs[idx].peaks.intensities[X_ind], \
                                                    self.bins, right=False))
            position_ids = torch.cat((position_ids, torch.zeros(self.max_len-len(position_ids), \
                                                                dtype=int)+self.max_len-1))
            # print(position_ids)
            #X = self.encode_spec_intens(np.array(self.ref_docs[idx].words)[X_ind], intens)
        X = self.encode_spec(np.array(self.ref_docs[idx].words)[X_ind])
        
#        y = torch.tensor([self.encode(peak) for peak in np.array(self.ref_docs[idx].words)[y_ind]])
        att_mask = torch.zeros(self.max_len, dtype=int)
        att_mask[:len(X)] = 1
        X_padded = torch.cat((X, torch.zeros(self.max_len-len(X), dtype=int)))
        if self.add_intensity:
...
class FixedSizeDS(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # mask_missing = self.rng.uniform(0,1, his_size) < self.p
